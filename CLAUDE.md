...
- `update-traffic.mjs` — Node ESM script, no dependencies, requires Node 24+ (uses built-in `fetch`).
- `update-traffic.test.mjs` — `node:test` suite for `formatMessage` and `mergeDaily` (the only pure functions). Integration is covered by `workflow_dispatch`.
- `.github/workflows/traffic-stats.yml` — daily cron (`30 0 * * *` UTC) plus `workflow_dispatch`.
- `.github/workflows/test.yml` — runs `node --test` on push and PR.
...
This repo dogfoods itself: the badges in `README.md` point at this repo's own `_stats_badges` branch. If you change the badge JSON schema or filename, update the README URLs too.
